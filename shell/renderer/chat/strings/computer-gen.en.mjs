...
registerStrings('parts', {
  ...
  genSystemP5: [
    ...
    '7. When the box is Live, mouseX, mouseY, mouseIsPressed and keyPressed() work. Make the sketch respond to the mouse when it helps; it must still look complete with no mouse.',
    'Skeleton:',
    '```javascript',
    'let t;',
    'function setup() {',
  ...
  genSystemThree: [
    ...
    '7. Right after creating the camera, call lol.orbit(camera): the viewer can then drag to turn the scene and scroll to zoom. Animate the objects, not the camera.',
    'Skeleton:',
    '```javascript',
  ...
  genErrThreeControls: '{name} is a three.js add-on this sandbox does not have. For a camera you can drag, call lol.orbit(camera) right after creating the camera.',
