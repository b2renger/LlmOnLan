...
const MODULES = [
  ...
  // K6 (addendum KF-5, KF-7). `media` keeps the files a box holds on THIS computer; `drops` turns a
  // file dropped on the canvas into the right box. `drops` installs AFTER `media` and `intake`,
  // whose doors it uses. Both are features: without `drops` a canvas drop is a graph file, as
