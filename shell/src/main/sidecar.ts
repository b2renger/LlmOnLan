...
export class SidecarSupervisor extends EventEmitter {
    ...
    async start(opts: { endpoint: string | null; dataDir: string; apiKey?: string | null; defaultModel?: string | null; searxngUrl?: string | null }): Promise<void> {
        ...
        const env = {
            ...process.env,
            ...buildSidecarEnv({ endpoint: this.endpoint, dataDir: this.dataDir, apiKey: this.apiKey, defaultModel: this.defaultModel, searxngUrl: this.searxngUrl }),
    ...
    async repoint(endpoint: string | null, apiKey: string | null = null, defaultModel: string | null = null, searxngUrl: string | null = null): Promise<void> {
        if (endpoint === this.endpoint && apiKey === this.apiKey && defaultModel === this.defaultModel && searxngUrl === this.searxngUrl) return;
        console.log(`[sidecar] repoint ${this.endpoint} → ${endpoint} (model ${this.defaultModel} → ${defaultModel}, search ${this.searxngUrl} → ${searxngUrl})`);
        this.setState({ status: 'restarting', endpoint });
        await this.stop({ keepState: true });
        await this.start({ endpoint, dataDir: this.dataDir, apiKey, defaultModel, searxngUrl });
    }
    ...
    async setDataDir(dataDir: string): Promise<void> {
        ...
        await this.stop({ keepState: true });
        await this.start({ endpoint: this.endpoint, dataDir, apiKey: this.apiKey, defaultModel: this.defaultModel, searxngUrl: this.searxngUrl });
    }
    ...
    private onChildExit(child: ChildProcess, code: number | null): void {
        ...
        if (this.crashRestarts < MAX_CRASH_RESTARTS) {
            ...
            this.start({ endpoint: this.endpoint, dataDir: this.dataDir, apiKey: this.apiKey, defaultModel: this.defaultModel, searxngUrl: this.searxngUrl });
        } else {
