# Vendored from Ollama-OCR (https://github.com/imanoop7/Ollama-OCR), MIT License,
# Copyright (c) 2024 Anoop Maurya. Bundled here (rather than `pip install ollama-ocr`)
# to avoid the package's heavy/awkward deps for a headless service — notably
# `python-magic` (needs native libmagic; a Windows landmine) plus streamlit and
# transformers, none of which the core OCRProcessor path imports. The class below is
# the upstream `src/ollama_ocr/ocr_processor.py` verbatim EXCEPT: (1) the debug
# `print()` lines were removed (they would spam the farm's [extract] log); (2) a
# `request_timeout` was added on the Ollama HTTP calls — upstream omits it, which
# lets a hung Ollama block a threadpool worker forever and, with enough concurrent
# hangs, stall the whole service. Real runtime deps: requests, pymupdf,
# opencv-python(-headless), numpy, tqdm.
#
# It talks to Ollama's NATIVE /api/generate (not the OpenAI /v1 proxy) with a
# vision model, driven by per-format prompts. See LICENSE in this directory.
...
class OCRProcessor:
    def __init__(self, model_name: str = "llama3.2-vision:11b",
        ...
        # (connect, read) seconds for the Ollama call. A dead/stalled Ollama hits
        # this instead of blocking forever; a Timeout is an Exception, so it lands in
        # the except below → "Error processing image: …" → server.py surfaces a 502
        # and frees the worker. read is generous (a non-stream vision generate sends
        # nothing until done, so this bounds total generation time on a page).
        ...
        try:
            ...
            if image_path.lower().endswith('.pdf'):
                ...
                for idx, page_file in enumerate(image_pages):
                    ...
                    response = requests.post(self.base_url, json=payload, timeout=self.request_timeout)
